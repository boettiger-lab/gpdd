...
The data is broken into seven text/csv files: data, main, timeperiod, taxon, datasource, biotope, and location, all available through the KNB website above. Our package provides a convenient way to download the datasets through R. 


## Using the Package

The package contains only one function `download_gpdd`, which takes in only one argument `dataset_name`. This argument should be one of "data", "main", "timeperiod", "taxon", "datasource", "biotope", "location". Other strings or non-string inputs will raise an error. Please also make sure to install `readr` before using this package. An example would be:
