...
test_that("Do not download when requested dataset exists in the directory", {
  ...
  expect_message(download_gpdd("gpdd_data", dir = test_path),
                 "gpdd_data already exists. Proceeding to the next dataset.")
...
test_that("Call download_gpdd when some datasets are present while some are not", {
  ...
  expect_message(download_gpdd(c("gpdd_data", "gpdd_main"), dir = test_path),
                 "gpdd_data already exists. Proceeding to the next dataset.")
  path_main = file.path(test_path, "gpdd_main.csv")
  expect_true(file.exists(path_data))
  expect_true(file.exists(path_main))
  ...
  expect_message(download_gpdd(
        ...
        dir = test_path), "gpdd_data already exists. Proceeding to the next dataset.")
  expect_message(download_gpdd(
    c("gpdd_data", "gpdd_main", "gpdd_timeperiod"),
    dir = test_path), "gpdd_main already exists. Proceeding to the next dataset.")
  expect_message(download_gpdd(
    c("gpdd_data", "gpdd_main", "gpdd_timeperiod"),
    dir = test_path), "gpdd_timeperiod already exists. Proceeding to the next dataset.")
